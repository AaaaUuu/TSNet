...
class OneModel(nn.Module):
    def __init__(self, args, cls_type=None,norm_layer=nn.BatchNorm2d):
        ...
        self.init_merge = nn.Sequential(
            StripPooling(reduce_dim* 2 + mask_add_num, (20, 12), norm_layer),
            SELayer(reduce_dim*2+ mask_add_num, reduce_dim* 2 + mask_add_num),
        ...
        self.cls_meta = nn.Sequential(
            TwinTransformer(dim=reduce_dim, depth=8),
            SELayer(reduce_dim, reduce_dim),
            nn.Conv2d(reduce_dim, reduce_dim, kernel_size=3, padding=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Dropout2d(p=0.1),
    ...
    def forward(self, x, s_x, s_y, y_m, y_b, cat_idx=None):
        ...
        # output, index = torch.max(final_out, dim=1)  # shape为[1,33,33]
