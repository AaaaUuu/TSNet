...
def get_parser():
    ...
    parser.add_argument('--viz', action='store_true', default='F:/TSNet/exp/pascal/TSNet/split0/resnet50/')
    parser.add_argument('--config', type=str, default='config/pascal/pascal_split0_resnet50.yaml', help='config file') # coco/coco_split0_resnet50.yaml
    parser.add_argument('--local_rank', type=int, default=-1, help='number of cpu threads to use during batch generation')
    parser.add_argument('--output-dir', default="F:/TSNet/output")
...
def train(train_loader, val_loader, model, optimizer, epoch):
    ...
    for i, (input, target, target_b, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        # output1=output.unsqueeze(0)
        output = np.max(output.cpu().numpy(), axis=1).reshape(113, 113)  # shape为[513,513]，二维
        output = (
                    ((output - np.min(output)) / (np.max(output) - np.min(output))) * 255).astype(
            np.uint8)
        # savedir = 'F:/TSNet/output/'
        # if not os.path.exists(savedir + 'val_pred_temp'):
        #     os.mkdir(savedir + 'val_pred_temp')
        # output1 = cv2.applyColorMap(output1, cv2.COLORMAP_JET)  # 伪彩色处理
        # if 70000>i>50000:
        #     cv2.imwrite(savedir + 'val_pred_temp/' + str(i) + '.jpg', output1)  # 保存可视化图像
